...
class AppJsonClient
{
    ...
    {
        $query = (new JsonAppSign())->sign($query, $this->appid, $this->secret);
        try {
    ...
    {
        $form_params = (new JsonAppSign())->sign($params, $this->appid, $this->secret);
        $multipart   = [];
    ...
    {
        $form_params = (new JsonAppSign())->sign($form_params, $this->appid, $this->secret);
    ...
    {
        $params = (new JsonAppSign())->sign($params, $this->appid, $this->secret);
    ...
    private function log($url, $method, $params, $resp): void
    {
        ...
        sys_info('poppy.ext-app-json-log', [
            'url'    => $url,
            'method' => $method,
            'params' => $params,
            'resp'   => $resp,
    ...
    private function handleException($e, string $url, string $method, array $params): array
    {
        ...
        sys_error('poppy.ext-app-json-log', [
            'url'       => $url,
            'method'    => $method,
            'params'    => $params,
            'exception' => $data,
