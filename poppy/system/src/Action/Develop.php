...
class Develop
{
    ...
    {
        $this->appid  = (string) env('KR36_DEVELOP_APPID');
        $secret       = (string) env('KR36_DEVELOP_SECRET');
        $this->host   = (string) env('KR36_DEVELOP_URL');
        $this->client = (new AppJsonClient())->setAppid($this->appid)->setSecret($secret)->enableLog();
    }
    ...
    public function generateSecret(): bool
    {
        ...
        $url    = $this->host . '/api/app/op/project/save-secret';
    ...
    {
        if (!$this->appid) {
            return $this->setError('当前未设置开发平台应用, 不进行上报');
        }
        return true;
    }
}