...
class RoleTest extends TestCase
{
    ...
    public function testEstablish(): void
    {
        ...
        $this->assertNotTrue(sys_tag('py-core-rbac')->exists(PyCoreDef::rbacCkRolePermissions($role->id)), '存在角色权限缓存标签');
        ...
        if ($Role->delete($id)) {
            $this->assertNotTrue(sys_tag('py-core-rbac')->exists(PyCoreDef::rbacCkRolePermissions($id)), '存在角色权限缓存标签');
        }
        else {
