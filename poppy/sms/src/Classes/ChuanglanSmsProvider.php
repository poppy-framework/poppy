...
class ChuanglanSmsProvider extends BaseSms implements SmsContract
{
    ...
    private function initConfig($mobiles): void
    {
        if (!UtilHelper::isChMobile($mobiles)) {
            $apiAccount  = (string) sys_setting('py-sms::sms.chuanglan_access_key');
            $apiPassword = (string) sys_setting('py-sms::sms.chuanglan_access_secret');
        }
        else {
            $apiAccount  = (string) sys_setting('py-sms::sms.chuanglan_cty_access_key');
            $apiPassword = (string) sys_setting('py-sms::sms.chuanglan_cty_access_secret');
        }
